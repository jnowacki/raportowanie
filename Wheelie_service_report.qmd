...
Income <- Income %>% mutate(production_year = as.integer(production_year),
                  payment_year = as.integer(substr(payment_deadline,1,4)),
...
Number of service activities in years 2016-2022 - split by **car model**.
...
g05 <- ggplot(df05, aes(x = as.character(calendar_year_month),
    ...
    geom_text(position = position_stack(vjust = 0.85), color = my_pal_2[2], size=3) +
    geom_text(position = position_stack(vjust = 0.85), color = my_pal_2[2], size=3.01) +
    geom_text(position = position_stack(vjust = 0.85), color = my_pal_2[2], size=3.02) +
    geom_text(position = position_stack(vjust = 0.85), color = my_pal_2[2], size=3.03) +
...
1.  *ile kosztuje nas serwis poszczególnych marek samochodów*
